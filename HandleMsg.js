...
module.exports = HandleMsg = async (aruga, message) => {
	...
	// Filter Banned People
        ...
        case 'ownerbot':
            ...
            .then(() => aruga.sendText(from, 'Jika kalian ingin request fitur silahkan chat nomor owner kami!'))
            break
        case 'join':
            ...
            if (isOwnerBot) {
                await aruga.joinGroupViaLink(linkgrup)
                      .then(async () => {
                          ...
                          await aruga.sendText(chekgrup.id, `Hai minna~, Im Bot Aryo Dewantoro untuk melihat menu bot kirim *${prefix}menu*`)
                      })
            } else {
                let cgrup = await aruga.getAllGroups()
                if (cgrup.length > groupLimit) return aruga.reply(from, `Maaf, grup pada Bot ini penuh\nGrup Maks: ${groupLimit}`, id)
                if (cgrup.size < memberLimit) return aruga.reply(from, `Maaf, Bot keluar jika anggota grup tidak melebihi ${memberLimit} anggota`, id)
                await aruga.joinGroupViaLink(linkgrup)
                      .then(async () =>{
	...
	case 'stimg':
            if (quotedMsg && quotedMsg.type == 'sticker') {
                ...
                await aruga.sendFile(from, imageBase64, 'imgsticker.jpg', '*_Success_*', id)
                .then(() => {
                    console.log(`Sticker to Image Processed for ${processTime(t, moment())} Seconds`)
                })
        } else if (!quotedMsg) return aruga.reply(from, `Format salah, silahkan tag/reply sticker yang ingin dijadikan gambar!`, id)
        break
        ...
        case 'stiker':
            ...
            case 'antistiker':
                    if (!isGroupMsg) return aruga.reply(from, 'Maaf, perintah ini hanya dapat dipakai didalam grup!', id)
                    if (!isGroupAdmins) return aruga.reply(from, 'Gagal, perintah ini hanya dapat digunakan oleh admin grup!', id)
                    if (!isBotGroupAdmins) return aruga.reply(from, 'Gagal, silahkan tambahkan bot sebagai admin grup! [Bot not Admin]', id)
                    if (args[0] == 'on') {
    ...
    case 'qrcode':
      ...
      break			
        ...
        case 'nulis':
            ...
            await aruga.sendImage(from, `${nulisp}`, '', '*_Success_*', id)
            .catch(() => {
                aruga.reply(from, 'Ada yang Error!', id)
            })
            break
        ...
        case 'audio':
        case 'alaudio':
            if (args.length == 0) return aruga.reply(from, `*_${prefix}ALaudio <nama surah>_*\nMenampilkan tautan dari audio surah tertentu. Contoh penggunaan : ${prefix}audio al-falaq\n\n*_${prefix}audio <nama surah> <ayat>_*\nMengirim audio surah dan ayat tertentu beserta terjemahannya dalam bahasa Indonesia. Contoh penggunaan : ${prefix}audio al-falaq 1\n\n*_${prefix}audio <nama surah> <ayat> en_*\nMengirim audio surah dan ayat tertentu beserta terjemahannya dalam bahasa Inggris. Contoh penggunaan : ${prefix}audio al-falaq 1 en`, message.id)
              ayat = "ayat"
              bhs = ""
                var responseh = await axios.get('https://raw.githubusercontent.com/ArugaZ/grabbed-results/main/islam/surah.json')
	...
	if (!isBotGroupAdmins) return aruga.reply(from, 'Perintah ini hanya bisa di gunakan ketika bot menjadi admin', id)
        ...
        case 'anime':
            ...
            if (args[0] == 'random' || args[0] == 'waifu' || args[0] == 'husbu' || args[0] == 'neko') {
                ...
                .then(body => {
                    ...
                    aruga.sendFileFromUrl(from, randomnimex, '', '*_Success_*', id)
                })
                .catch(() => {
                    aruga.reply(from, 'Ada yang Error!', id)
                })
        ...
        case 'kpop':
            ...
            if (args[0] == 'blackpink' || args[0] == 'exo' || args[0] == 'bts') {
                ...
                .then(body => {
                    ...
                    aruga.sendFileFromUrl(from, randomkpopx, '', '*_Success_*', id)
                })
                .catch(() => {
                    aruga.reply(from, 'Ada yang Error!', id)
                })
        ...
        case 'cuaca':
            ...
            break
        case 'infogempa':
            const bmkg = await get.get(`https://mhankbarbar.tech/api/infogempa?apiKey=Rk0ZljTfvs2kBB9S3EWL`).json()
    ...
    if (!isGroupMsg) return aruga.reply(from, menuId.textPrem())
        ...
        case 'bc': //untuk broadcast atau promosi
            ...
            for (let idk of chatz) {
                ...
                if (!cvk.isReadOnly) aruga.sendText(idk, `〘 *BOT ARYO DEWANTORO* 〙\n\n${msg}`)
                if (cvk.isReadOnly) aruga.sendText(idk, `〘 *BOT ARYO DEWANTORO* 〙\n\n${msg}`)
            }
